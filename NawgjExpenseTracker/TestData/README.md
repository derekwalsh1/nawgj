...
## App Store screenshot / release test data (newest)

For App Store previews/screenshots and general manual testing, use the files
generated by `generate_release_test_data.py`:

- **`TestJudges.json`** - a 40-judge roster (name + level only) for
  "Import Judges" on the Judge List screen.
- **`TestMeetLarge.json`** - a 3-day meet (25 judges) with multiple sessions
  per day, including overlapping sessions (e.g. a "Compulsory Session" and
  "Xcel Session" running concurrently on day 1; 3 overlapping sessions on
  day 2).
- **`TestMeetMedium.json`** - a single-day meet (12 judges) with 3
  overlapping sessions.
- **`TestMeetSmall.json`** - a single-day, single-session meet (5 judges).

Each is a standalone file - import `TestJudges.json` via the Judge List's
"Import" action, and each `TestMeet*.json` via the Meet List's "Import"
action. Every judge in the generated meets has all 8 expense categories
present (zeroed out where not applicable) and a fee entry for every session
in the meet (excluded/zero-hour for sessions they didn't work), so every
row in the app's Fees/Expenses screens is populated and tappable.

Regenerate with:
```bash
cd NawgjExpenseTracker/TestData
python3 generate_release_test_data.py
```
